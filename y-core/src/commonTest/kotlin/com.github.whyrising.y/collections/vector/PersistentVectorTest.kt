...
class PersistentVectorTest : FreeSpec({
    ...
    "PersistentVector" - {
        ...
        "asTransient() should turn this vector into a transient vector" {
            ...
            val tVec: PersistentVector.TransientVector<Int> = vec.asTransient()
        ...
        "seq()" - {
            ...
            "when called on a filled vector, it should return a ChunkedSeq" {
                ...
                val chunkedSeq = vec.seq() as PersistentVector.ChunkedSeq<Int>
        ...
        "ChunkedSeq" - {
            "firstChunk()" {
                val vec = PersistentVector(1, 2, 3)
                val chunkedSeq = PersistentVector.ChunkedSeq(vec, 0, 0)
            ...
            "restChunks()" {
                ...
                val chunkedSeq = PersistentVector.ChunkedSeq(vec, index, offset)
                val node = vec.leafArrayBy(0)

                val restChunks =
                    chunkedSeq.restChunks() as PersistentVector.ChunkedSeq<Int>
            ...
            "rest()" {
                ...
                val rest = chunkedSeq.rest() as PersistentVector.ChunkedSeq<Int>
                var nextChunk = rest
                var i = 1
                while (i <= 31) {
                    nextChunk =
                        nextChunk.rest() as PersistentVector.ChunkedSeq<Int>
                    i++
                }
