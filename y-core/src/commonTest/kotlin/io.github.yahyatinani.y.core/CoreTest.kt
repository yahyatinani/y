...
class CoreTest : FreeSpec({
  ...
  "assoc(map, key, val, kvs)" {
    ...
    assoc(null, ":a" to 15, ":b" to 20) shouldBe m(
      ":a" to 15,
      ":b" to 20,
    )
  ...
  "m(vararg any)" - {
    "when array empty, return empty an map" {
      io.github.yahyatinani.y.core.util.m<String, Int>() shouldBeSameInstanceAs
        EmptyArrayMap
    }

    "when array size <= HASHTABLE_THRESHOLD, return PersistentArrayMap" {
      io.github.yahyatinani.y.core.util.m<String, Int>("a", 1) shouldBe
        m("a" to 1)

      io.github.yahyatinani.y.core.util.m<String, Int>(
        "a", 1,
        "b", 1,
        "c", 1,
        "d", 1,
