...
class OptionTest : FreeSpec({
    ...
    "hLift" - {
        ...
        "should convert f: A -> B -> C to g: A -> B -> Option(C)" - {
            checkAll { n: Int, l: Long ->
                val f: (Int) -> (Long) -> String = { { m -> str(it, m) } }

                val g: (Int) -> (Long) -> Option<String> = hLift(f)

                g(n)(l) shouldBe Option(f(n)(l))
            }
        }

        "should convert f: A -> B -> C -> D to g: A -> B -> C -> Option(D)" - {
            checkAll { n: Int, l: Long, d: Double ->
                val f: (Int) -> (Long) -> (Double) -> String =
                    { { m -> { d -> str(it, m, d) } } }

                val g: (Int) -> (Long) -> (Double) -> Option<String> = hLift(f)

                g(n)(l)(d) shouldBe Option(f(n)(l)(d))
            }
        }
    }
    ...
    "map package level" - {
        ...
        "Option(A), Option(B), Option(C) and f:(A)->(B)->(C)->D to Option(D)" {
            checkAll { n: Int, l: Long, d: Double ->
                val option1 = Option(n)
                val option2 = Option(l)
                val option3 = Option(d)
